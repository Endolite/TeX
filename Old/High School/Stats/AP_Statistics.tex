\documentclass[12pt, A4]{report}

\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{arydshln}
\usepackage{bm}
\usepackage{csquotes}
\usepackage[hang,flushmargin]{footmisc} 
\usepackage[margin=0.5in]{geometry}
\usepackage[utf8]{inputenc}
\usepackage{imakeidx}
\usepackage{moresize}
\usepackage{multirow}
\usepackage{pgfplots}
\usepackage{tikz}
\usetikzlibrary{calc}

\indexsetup{level = \chapter, toclevel = chapter}

\DeclareMathOperator{\B}{\mathrm{B}}
\DeclareMathOperator{\binomcdf}{\texttt{binomcdf}}
\DeclareMathOperator{\binompdf}{\texttt{binompdf}}
\DeclareMathOperator{\crit}{\text{critical value}}
\DeclareMathOperator{\df}{\mathrm{df}}
\DeclareMathOperator{\erf}{erf}
\DeclareMathOperator{\invNorm}{\texttt{invNorm}}
\DeclareMathOperator{\ME}{ME}
\DeclareMathOperator{\moe}{\text{margin of error}}
\DeclareMathOperator{\nint}{nint}
\DeclareMathOperator{\np}{\text{null parameter}}
\DeclareMathOperator{\normalcdf}{\texttt{normalcdf}}
\DeclareMathOperator{\os}{\text{observed statistic}}
\DeclareMathOperator{\p}{parameter}
\DeclareMathOperator{\pest}{\text{point estimate}}
\DeclareMathOperator{\randomnorm}{\texttt{randomnorm}}
\DeclareMathOperator{\s}{statistic}
\DeclareMathOperator{\se}{\text{standard error}}
\DeclareMathOperator{\ts}{\text{test statistic}}
\DeclareMathOperator{\sts}{\text{standardized test statistic}}

\newcommand*{\approxident}{\mathrel{\vcenter{\offinterlineskip\hbox{$\sim$}\vskip-.35ex\hbox{$\sim$}\vskip-.35ex\hbox{$\sim$}}}}
\newcommand{\binomCDF}[3]{\binomcdf\left(n:#1, p:#2, x:#3\right)}
\newcommand{\binomPDF}[3]{\binompdf\left(n:#1, p:#2, x:#3\right)}
\newcommand{\callout}[2]{\begin{center}\fbox{\begin{minipage}{#1cm}#2\end{minipage}}\end{center}}
\newcommand{\Chi}{X}
\newcommand{\cint}{\text{confidence interval}}
\newcommand{\comment}[1]{}
\renewcommand{\d}{\,\mathrm{d}}
\newcommand{\diff}[1]{#1_{\mathrm{diff}}}
\newcommand{\emphi}[1]{\emph{#1}\index{#1}}
\newcommand{\invNormal}[4]{\invNorm\left(\mathrm{area}:#1, \mu: #2, \sigma: #3, \mathrm{Tail: #4}\right)}
\newcommand{\mpmeandiff}[2]{\mu_{\bar{x}_#1 - \bar{x}_#2}}
\newcommand{\msdiff}[3]{#1_{\bar{x}_#2 - \bar{x}_#3}}
\newcommand{\msdiffe}[5]{\sqrt{\frac{#1_#2^2}{#3} + \frac{#1_#4^2}{#5}}}
\newcommand{\normalCDF}[4]{\normalcdf\left(\mathrm{lower}: #1, \mathrm{upper}: #2, \mu: #3, \sigma: #4\right)}
\newcommand{\pindex}[1]{#1\index{#1}}
\newcommand{\ppmeandiff}[2]{\mu_{\hat{p}_#1 - \hat{p}_#2}}
\newcommand{\propse}[2]{\sqrt{\frac{#1\left(1 - #1\right)}{#2}}}
\newcommand{\propsed}[4]{\sqrt{\frac{#1\left(1 - #1\right)}{#2} + \frac{#3\left(1 - #3\right)}{#4}}}
\newcommand{\propsee}[3]{\sqrt{#1\left(1 - #1\right)\left(\frac{1}{#2} - \frac{1}{#3}\right)}}
\newcommand{\randnorm}[3]{\randomnorm\left(\mu: #1, \sigma: #2, n: #3\right)}
\renewcommand{\Roman}[1]{\MakeUppercase{\romannumeral #1}}
\newcommand{\textbfi}[1]{\textbf{#1}\index{#1}}
\newcommand{\pval}{P\text{-value}}
\newcommand{\z}[3]{\frac{#1 - #2}{#3}}

\makeindex
\usepackage{subfiles}

\title{AP Statistics}
\author{Arnav Patri}

\begin{document}
	\maketitle
	\tableofcontents
	\part{One-Variable Data}
		\subfile{units/1}
	\part{Two-Variable Data}
		\subfile{units/2}
	\part{Collecting Data}
		\subfile{units/3}
	\part{Probability, Random Variables, and Probability Distributions}
		\subfile{units/4}
	\part{Sampling Distributions}
		\subfile{units/5}
	\part{Inference}
		\subfile{units/6}
	\part{Back Matter}
		\printindex
\end{document}