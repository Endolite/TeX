...
\begin{document}
	...
	\section{Initial-Value Problems}
		Often, a solution to a DE must meet other conditions imposed on it and its derivatives. \\
			...
			If these desired values are constants, this is an \textbf{\(\bm{n^\th}\)-order initial-value problem (IVP)}. The desired values are called \textbf{initial conditions (IC)}. \\
		...
		\subsectionb{Existence and Uniqueness}
			...
			\callout{17}{
				Let \(R\) be a rectangular region in the \(xy\)-plane defined by
					...
					that contains point \((x_0, y_0)\). If \(f(x, y)\) and \(\pdv*{f}{y}\) are continuous over \(R\), then there exists some interval \(I_0: (x_0 - h, x_0 + h)\) (where \(h > 0\)) contained in \([a, b]\) and a unique function \(y(x)\) defined on this interval that is a solution of the IVP.
			}
\end{document}