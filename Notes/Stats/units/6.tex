...
\begin{document}
	\chapter{Confidence Intervals}
		...
		The margin of error describes the maximum deviation of the estimate from the parameter. It is the product of the \textbf{critical value} $\pmb{z}^*$ and the standard error of the statistic.
		$$ME = z^*s$$
		$z^*$ is equal to the absolute value of the $z$-score for which the probability of $Z$ being within $\pm z^*$ of 0 is $C$.\footnote{This can be calculated using $\invNorm$:$$$$}
		$$P(-z < Z < z) = C$$
		...
		\section{Confidence Intervals about Means}
			...
			The specific $t$ distribution used is specified by \textbf{degrees of freedom} ($\pmb{\df}$, which is 1 less than the sample size. \\
			$$\df = n - 1$$
