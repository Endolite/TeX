...
\begin{document}
	...
	\chapter{Random Variables}
		...
		\[P(X = x_i) = P(x_i) = p_i\]
		The \emphi{mean} or \textbfi{expected value} (denoted $\mathbb{E}(X)$) is weighted average of all possible values of $X$.
	...
	\chapter{Probability Distributions}
		\section*{Bernoulli Distributions}
			The \textbf{Bernoulli distribution} is a probability distribution of a \textbfi{binary} random variable, a variable that can only take 1 of 2 values, a 0 or a 1. \\
		...
		\section{Binomial Distributions}
			\callout{17}{
				...
				\begin{enumerate}
					\item
						The possible outcomes of each trial must be \emph{binary}.
					\item
			...
			\[\sigma_X^2 = np(1 - p)\]
			A binomial distribution\index{distribution!binomial} can be graphed as a \pindex{histogram}.\footnote{A binomial distribution's graph can be thought of as a piecewise.\[f(x; n, p) = \begin{cases}\binom{n}{x}(1 - p)^{n - x}p^x & x \in \mathbb{N} \land x \le n\\ f(\nint(x); n, p) & x \notin \mathbb{N} \land x \le n\end{cases}\]} \\
