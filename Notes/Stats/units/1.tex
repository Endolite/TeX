...
\begin{document}
	\chapter{One-Variable Quantitative Data}
		...
		Data provides values for \textbf{variables}\index{variables}, which describe some characteristic of an \emph{individual}\index{individual}. \\
		...
		A variable's \textbf{distribution}\index{distribution} describes the \emph{frequency}\index{frequency} with which a variable takes on its possible values.
		\section{Graphically Displaying Distributions}
			...
			A \textbf{cumulative frequency curve}\index{cumulative!frequency curve} (OGIVE)\index{OGIVE} is a line graph that shows the cumulative relative frequency, the sum of all lower classes' \textbf{relative frequencies}\index{frequency!relative}, the percentages of data contained within each class, (inclusive). Rather than each class being represented as a range, they are labeled by their \emph{medians}\index{median}.
			\subsection*{Graphically Describing Distributions}
				A distribution can be described by its \emph{shape}\index{shape}, \emph{outliers}\index{outlier}, \emph{center}\index{measure of!center}, and \emph{spread}\index{measure of!spread} (SOCS).
				\paragraph{Shape}
					...
					\begin{center}
						...
						\end{tikzpicture}
					\end{center} 
					\textbf{Skew}\index{skew!left}\index{skew!right} is dependent on the direction of the tail\index{tail}.
					\begin{align*}
					...
					\end{align*}
				\paragraph{Outliers}
					An \textbf{outlier}\index{outlier} is a point that does not fit the general trend of the data\index{data}. It is marked as an asterisk on a modified box plot\index{plot!box!modified}. \\
				\paragraph{Center}
					The \emph{mean}\index{mean} ($\bar{x}$), \emph{median}\index{median} ($Q_2$), and \emph{mode}\index{mode} are \text{measures of center}\index{measure of!center}, meaning that they measure where the data is centered. The relationship between these quantities is related to the skew.
					\[\begin{array}{|c|c:c:c|}\hline
						\rm{Skew} & \rm{Left} & \rm{Center} & \rm{Right} \\\hline
						\rm{Relationship} & \bar{x} < Q_2 < \rm{mode} & \bar{x} = Q_2 = \rm{mode} & \bar{x} > Q_2 > \rm{mode} \\\hline
					\end{array}\]
				\paragraph{Spread}
					The \emph{standard deviation}\index{standard deviation}, \emph{range}\index{range}, and \emph{interquartile range}\index{range!interquartile} are measures of \textbf{spread}\index{spread}, meaning that they measure how spread apart the data is. The greater they are, the more variability there is in the data\index{data}.
		\section{Numerically Describing Distributions}
			The \textbf{mean}\index{mean} is the sum of each value (not necessarily unique) in the distribution\index{distribution} divided by the total number of values. It is also referred to as the \emph{average}\index{average} or \emph{expected value}\index{expected value} of its variable\index{variable}. For a sample, it is denoted using a bar over the lowercase form of its variable ($X$ becoming $\pmb{\bar{x}}$).
			\[\bar{x} = \frac{\sum x_i}{n}\]
			The first, second, and third \textbf{quartiles}\index{quartiles}, denoted $\pmb{Q_1}$, $\pmb{Q_2}$, and $\pmb{Q_3}$, are the values with 25\%, 50\%, and 75\% of the data below them respectively.\footnote{The probability\index{probability} of $X$ falling below each quartile is equal to its number multiplied by 25\%. \begin{align*} P(X < Q_1) &= 0.25 & P(X < Q_2) &= 0.5 & P(X < Q_3) &= 0.75\end{align*}} The second quartile is the \textbf{median}. \\
			...
			\[IQR = Q_3 - Q_1\]
			The \textbf{standard deviation}\index{standard deviation} is the average distance from the mean\index{mean}. It is denoted by $\pmb{s}$  with the subscript of its variable's lowercase form (for a sample). Along with the range and interquartile range, it is a measure of spread/variability.
			...
			An \emph{outlier}\index{outlier} is defined as a point that is more than 1.5 times the interquartile range from the \enquote{middle half} of the data.
			\[x_i\notin (Q_1 - 1.5IQR, Q_2 + 1.5IQR) \implies x_i \text{ is an outlier}\]
			\callout{17}{It is crucial to note that the mean, standard deviation, variance, and range are only to be used when there are no outliers, as outliers disproportionately affect them. The median, mode, and interquartile range can always be used, though, as they are \textbf{resistant}\index{resistant}.}
			A value's \textbf{percentile}\index{percentile} is the percentage of all values that are at less than or equal to it. This is simply the probability\index{probability} of the variable being at most the value. \\
		...
		\section{Density Curves}
			A \textbf{density curve}\index{density curve}, typically denoted $\pmb{f}$, displays the \enquote{\emph{relative frequencies}}\index{relative frequencies} of every possible value of its variable\index{variable}. As such, the total area under the curve must be exactly 1 and its values must always be positive. \\ 
			A density curve is a \emph{continuous}\index{continuous} distribution, so the probability of $X$ being any particular value is 0.\footnote{Probability is equal to the integral over the desired interval, so when both bounds are the same, it will be equal to zero.\[\int_x^xf(t)\d t = F(x) - F(x) = 0\]}
			\callout{18.4}{Because the probability of $X$ being any particular value is 0, the function's output values do not actually represent anything, though they can be likened to the relative frequencies of distribution curves.}
			...
			\[P(X \le x) = F(x)\]
			\callout{17}{As a density curve is categorically continuous, the inclusivity of the binary relation of the probability statement is irrelevant to the value of the percentile.\footnote{The lack of regard for inclusivity with percentile, as well as other probability statements made regarding continuous distributions, is due to the area being $\int_{-\infty}^xf(t)\d t$. As $\mathrm{d}t$ is a differential, it is infinitesimal, so adding or subtracting $f(x)\d t$ to or from the integral has no tangible impact on its value.}}
